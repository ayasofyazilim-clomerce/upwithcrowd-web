...
export const middleware = auth((request: NextAuthRequest) => {
  ...
  function redirectToRoot(locale: string) {
    return NextResponse.redirect(new URL(`${hostURL}/${locale}/`, hostURL));
  }
  ...
  if (!isAuthorized && forbiddenURLs.includes(pathName)) {
  ...
  return NextResponse.redirect(
    new URL(`${hostURL}/${locale}${request.nextUrl.pathname}${request.nextUrl.search}`, hostURL),
  );
});
