  ...
  return (
    <div className="flex w-full flex-col">
      <div className="relative mt-2">
        <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500">{baseLink}</span>
        <Input
          type="text"
          id="profileLink"
          value={url}
      ...
      <p className="mt-2 flex items-center gap-2 text-sm text-gray-500">
        <CircleAlert /> Include at least one letter—you can also use numbers and dashes.
      </p>
      <div className="mt-3 flex justify-end">
        <Button
          type="button"
          disabled={url.length < 3}
          className={`${url.length < 3 ? "bg-muted text-muted-foreground" : "text-muted bg-black hover:bg-black/90"}`}>
          Confirm
        </Button>
      </div>
    </div>
